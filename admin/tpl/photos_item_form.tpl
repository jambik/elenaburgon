									...
									{else}
										<select class="input-select" name="{$value.field}" id="{$value.field}">
											...
											{foreach from=$keyValues[$value.field] key=key2 item=value2}
												<option value="{$value2[$value.field]}"{if $value2[$value.field] == $item[$value.field] || $value2[$value.field] == $smarty.get.gallery} selected="selected"{/if}>{$value2[$value.key_value]}</option>
											{/foreach}
										</select>
									{/if}
									{if $value.hint} <span class="hint">{$value.hint}</span>{/if}
						...
						{elseif $value.type=='set'}
							<tr>
								...
								<td>
									...
									{else}
										{foreach from=$setValues[$value.field] item=item2}
											<span class="set-item{if $item2.set_on} set-item-on{/if}" title="{$item2[$value.field]}" onclick="ToggleSetItem(this);">{$item2[$value.set_value]}</span>
										{/foreach}
									{/if}
									{if $value.hint} <span class="hint">{$value.hint}</span>{/if}
								</td>
